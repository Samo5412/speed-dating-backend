...
export const getSharedContactByContactId = async (
	...
	try {
		...
		if (!sharedContact) {
			res.status(404).json({
				message: MESSAGES.SHARED_CONTACT.NOT_FOUND,
			});
			return;
		}
...
export const addSharedContact = async (
	...
	try {
		...
		if (!contactUser) {
			res.status(404).json({
				message: MESSAGES.SHARED_CONTACT.CONTACT_NOT_FOUND,
			});
			return;
		}
		...
		if (existingContact) {
			res.status(400).json({
				message: MESSAGES.SHARED_CONTACT.ALREADY_EXISTS,
			});
			return;
		}
		...
		await user.save();
		res.status(201).json(
			user.sharedContacts[user.sharedContacts.length - 1]
		);
	} catch (error) {
		res.status(400).json({ message: error.message });
	}
...
export const createNotification = async (
	...
	try {
		...
		if (!message) {
			res.status(400).json({
				message: MESSAGES.NOTIFICATION.MESSAGE_REQUIRED,
			});
			return;
		}
